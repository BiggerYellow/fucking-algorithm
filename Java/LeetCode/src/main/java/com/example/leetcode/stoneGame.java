...
//        int[] piles = {3,7,4,3};
    ...
    //         先手玩家取前面piles[i],那么dp[i+1][j]表示后手玩家在这个区间内比先手玩家多的最大石子数,取两者之差: dp[i][j] = piles[i]-dp[i+1][j]
    //         先手玩家取后面piles[j],那么dp[i][j-1]表示后手玩家在这个区间内比先手玩家多的最大石子数,取两者之差: dp[i][j] = piles[j]-dp[i][j-1]
