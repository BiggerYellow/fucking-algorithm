...
//        String num ="214010";  //"974010"
//        int[] change = {6,7,9,7,4,0,3,4,4,7}; //"974676"
        String num ="5";  //"974010"
        int[] change = {1,4,7,5,3,2,5,6,9,4}; //"974676"
        System.out.println(maximumNumber(num, change));
    }

    public static String maximumNumber(String num, int[] change) {
        char[] chars = num.toCharArray();
        int flag = 0; //0-不变 1-相等 2-替换
        for (int i=0;i<num.length();i++){
            if ((num.charAt(i) - '0') > (change[num.charAt(i) - '0'])){
                ...
                flag= 2;
                int max = Math.max((num.charAt(i) - '0') , (change[num.charAt(i) - '0']));
                chars[i] = (char) (max+'0');
            }
